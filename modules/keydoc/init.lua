...
local function new(mod, key, press, release, docstring)
  ...
  -- if it's a string.
  if press and release and not docstring and type(release) == "string"
  then
    docstring = release
    release = nil
  end
...
local function key2str(key)
  ...
  if not key.modifiers or #key.modifiers == 0
  then
    return sym
  end
  ...
  for _, mod in pairs(key.modifiers)
  do
...
local function markup(keys)
  ...
  do
    ...
    then
      ...
      result[group] = (result[group] or "") .. '<span color="' .. beautiful.fg_focus .. '"> ' .. string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey .. '</span> <span color="' .. beautiful.fg_normal .. '">' .. help .. '</span>\n'
    end
  end
...
function display()
  ...
  for group, res in spairs(strings)
  do
    if #result > 0
    then
      result = result .. "\n"
